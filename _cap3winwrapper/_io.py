...
def _write_cap_sh(name,pathToCAP3,cap3Args,dictDes):
    """
    ...
    cap3Args: a file of CAP3 arguments to be supplied to each line
    ...
    lArg = []
    if cap3Args is not None:
        for line in open(cap3Args, 'r'):
            ls = line.strip()
            c = ls.find('#')
            if c != -1:
                lArg.append(ls[:c])
